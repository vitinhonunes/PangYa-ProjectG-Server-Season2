...
namespace PcapSniffer
{
    public sealed class ToClientBuffer
    {
        ...
        public List<byte[]> PutPacket(byte[] packet)
        {
            lock (_buffer)
            {
                ...
                while (true)
                {
                    ...
                    {
                        list.Add(result);
                        _buffer = _buffer.Skip(result.Length + 5).ToArray();
        ...
        private byte[] InternalProcessPacket()
        {
            ...
            var teste = _buffer.Skip(rawPacket.Length).ToArray();

            

            byte[] decryptedPacket = Shared.Server.Cryptor.Decrypt(rawPacket, _serverCryptKey);

            //var recrypted = Shared.Server.Cryptor.Encrypt(decryptedPacket, _serverCryptKey);
